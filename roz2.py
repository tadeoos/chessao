...
# (type e to exit)\n''').upper()
			...
			if last_card.ran!='K' or last_card.kol != 1: # po królu pik nie zagrywasz karty
				...
				hist_dict = {'gracz':gr,'kolej': kolej, 'szach': self.czy_szach(odwrot(kolej)),'last card':last_card,'now card': now_card, 'move': last_move, 'walet': walet, 'trojka': trojka, 'czworka': czworka, 'ok_zbicie': ok_zbicie, 'kpik':kpik, 'kkier':kkier, 'spalona': spalona, 'ominięta': False, 'bierka':None}
				if not spalona:
					# AS
					if kar[-1].ran == 'A':
			...
			hist_dict['bierka']=self.plansza.brd[self.plansza.mapdict[z[0]]].name
			if not spalona:
